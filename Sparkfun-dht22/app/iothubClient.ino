...
//void SendTemperatureIntervalToCloud() {
//    char messagePayload[MESSAGE_MAX_LEN];
...
void SendTemperatureToCloud() {
    char messagePayload[MESSAGE_MAX_LEN];
    createTemperatureMessage(messagePayload);
    sendMessage(iotHubClientHandle, messagePayload);
}

void SendHumidityToCloud() {
    char messagePayload[MESSAGE_MAX_LEN];
    createHumidityMessage(messagePayload);
    sendMessage(iotHubClientHandle, messagePayload);
}
...
void SetTemperatureInterval(float interval)
{
  temperatureInterval = (int)interval;
}

void SetHumidityInterval(float interval)
{
  humidityInterval = (int)interval;
}
...
IOTHUBMESSAGE_DISPOSITION_RESULT receiveMessageCallback(IOTHUB_MESSAGE_HANDLE message, void *userContextCallback)
{
    ...
    else
    {
        ...
        const char* charProperty = json["PropertyName"];
        String property = String(charProperty);

        if (property == "Temperature Measurement Interval")
          SetTemperatureInterval(value.toFloat());
        if (property == "Humidity Measurement Interval")
          SetHumidityInterval(value.toFloat());
