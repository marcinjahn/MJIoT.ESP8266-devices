...
void SendServoStateToCloud(int value) {
    char messagePayload[MESSAGE_MAX_LEN];
    createServoMessage(messagePayload, value);
    sendMessage(iotHubClientHandle, messagePayload);
}

void SendSensorStateToCloud(bool value) {
    char messagePayload[MESSAGE_MAX_LEN];
    createSensorMessage(messagePayload, value);
    sendMessage(iotHubClientHandle, messagePayload);
}

void setServo(int value) {
  servo.write(value);
  SendServoStateToCloud(value);
}
...
IOTHUBMESSAGE_DISPOSITION_RESULT receiveMessageCallback(IOTHUB_MESSAGE_HANDLE message, void *userContextCallback)
{
    ...
    else
    {
        ...
        if (intValue == 0 && value != "0") //0 is returned when value is not a number. Return in such case
        {
          free(temp);
          return IOTHUBMESSAGE_ACCEPTED;
        }
